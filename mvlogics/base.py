...
(_singleton_new := lambda cls: (c := _singleton_new.cache).get(cls, False) or c.__setitem__(cls, r := object.__new__(cls)) or r).cache, getcontext().prec = WeakKeyDictionary(), 100
_all, ALL_METHODS, ALL_LOGICS, FAKE_PROTOCOLS = ('ALL_METHODS', 'FORBIDDEN', 'ALL_LOGICS', 'ALL_LOGICS_TUPLE', 'FAKE_PROTOCOLS', 'FAKE_PROTOCOLS_TUPLE', 'REQUIRED_ATTRS', 'RECOMMENDED_METHODS', 'MIXIN_METHODS', 'EXTENSION_METHODS', 'protocols', 'is_logic', 'is_logic_member', 'is_builtin_logic', 'is_builtin_logic_member', 'convert', 'gödel_logic', 'łukasiewicz_logic', 'post_logic', 't_norm_logic', 'logic_from_implication', 'decimal_t_norm_logic', 'rational_t_norm_logic', 'decimal_logic_from_implication', 'rational_logic_from_implication', *(ALL_LOGICS_TUPLE := ('Unit', 'Boolean', 'K3', 'LP', 'BI3', 'RM3', 'G3', 'SmT', 'L3', 'P3', 'B4', 'Π', 'Π_aleph_0', 'G_inf', 'G_aleph_0', 'L_inf', 'L_aleph_0', 'NP', 'NP_aleph_0'))), REQUIRED_ATTRS.union(RECOMMENDED_METHODS := frozenset(('implies', '__bool__')), MIXIN_METHODS := frozenset(('iff', 'implies', '__reduce__', '__pos__', '__neg__', '__xor__', 'name', 'value', 'nand', 'nor', 'xnor', 'abjunction', 'converse_implies', 'converse_abjunction')), EXTENSION_METHODS := frozenset(('strong_disjunction', 'strong_conjunction', 'diamond', 'box', 'doubtful', 'gullibility', 'consensus'))), frozenset(ALL_LOGICS_TUPLE), frozenset(FAKE_PROTOCOLS_TUPLE := ('MemberlessLogicBase', 'AbstractLogicBase', 'LogicBase', 'InfiniteLogicBase', 'DecimalLogicBase', 'RationalLogicBase', 'StrictLogicBase', 'GödelLogic', 'ŁukasiewiczLogic', 'PostLogic', 'TNormLogic', 'RationalTNormLogic'))
class _SubmoduleMeta(type):
    def __new__(cls, name, bases, namespace, /, *, _supermodule_default_name_='<unknown>'):
...
class _InfLogicMetaBase(_AllLogicMeta):
    __defaults = _AllLogicMeta._AllLogicMeta__defaults|{'members': {}, '__repr__': _repr_cache(lambda self: f'{type(self).__name__}({self.value})')}
    @property
...
class _DecimalLogicMeta(_InfLogicMetaBase):
    ...
    def __new__(cls, val='0', /, _=_):
        ...
        return v
    __defaults = _InfLogicMetaBase._InfLogicMetaBase__defaults|{'__new__': __new__, 'value': property(_.__getitem__)}; del _, __new__
...
class _FakeProtocolMeta(type):
    ...
    def __new__(mcls, name, bases=(), namespace=None, /, **k):
        ...
        def __new__(*_): raise TypeError(f'cannot instantiate protocol {name!r}')
        if namespace is None: namespace = {}
        namespace.update(__init_subclass__=__init_subclass__, __new__=__new__)
